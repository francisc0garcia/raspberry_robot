...
class Model(SpatialEntity):
    ...
    def get_parent(self, requested_linkname, prefix='', visited=None):
        if visited is None:
            visited = []
...
class Joint(SpatialEntity):
    ...
    def add_urdf_elements(self, node, prefix):
        ...
        # in SDF a joint's pose is given in child link frame, in URDF joint frame = child link frame, i.e.
        # specifiy relative to parent joint (not parent link)
    ...
    def add_urdf_elements(self, node, modelCBTjoint):
        if (self.version <= 1.4) or (
            ...
            if xyz_norm != 0.0:
                ...
                # % (self.xyz, modelCBTjoint, rotation_modelCBTjoint, xyz_joint))
